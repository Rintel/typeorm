...
export class PostgresQueryRunner
    ...
    async renameTable(
        ...
        for (let column of enumColumns) {
            ...
            upQueries.push(
                new Query(
                    `ALTER TYPE "${oldEnumType.enumTypeSchema}"."${
                        oldEnumType.enumTypeName
        ...
        if (
            ...
            if (oldColumn.name !== newColumn.name) {
                ...
                if (
                    ...
                    const oldEnumType = await this.getEnumTypeName(
                        table,
                        oldColumn
                    );
                    upQueries.push(
            ...
            if (
                ...
                const oldEnumName = this.buildEnumName(
                    table,
                    newColumn,
                    true,
                    false,
                ...
                const enumTypeBeforeColumnChange = await this.getEnumTypeName(
                    table,
                    oldColumn
                );
                ...
                upQueries.push(
                    new Query(
                        `ALTER TYPE "${enumTypeBeforeColumnChange.enumTypeSchema}"."${enumTypeBeforeColumnChange.enumTypeName}" RENAME TO ${oldEnumNameWithoutSchema}`
                    )
                );
                downQueries.push(
                    new Query(
                        `ALTER TYPE ${oldEnumName} RENAME TO  "${enumTypeBeforeColumnChange.enumTypeName}"`
                    )
                );
                ...
                // remove old ENUM
                upQueries.push(
                    this.dropEnumTypeSql(table, newColumn, oldEnumName)
                );
                downQueries.push(
    ...
    async createExclusionConstraint(
        ...
        const up = this.createExclusionConstraintSql(
            table,
            exclusionConstraint
        );
        const down = this.dropExclusionConstraintSql(
            table,
            exclusionConstraint
        );
        await this.executeQueries(up, down);
    ...
    async dropExclusionConstraint(
        ...
        const exclusionConstraint =
            exclusionOrName instanceof TableExclusion
                ? exclusionOrName
    ...
    async createIndices(
        ...
        await PromiseUtils.runInSequence(indices, (index) =>
